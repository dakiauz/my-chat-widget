...
const FloatingDialer = React.memo(() => {
    ...
    const { callState, sessions, focusedCallSid, dialedNumber, isDialerOpen, connectionStatus, isSpeakerOn, showKeypadInCall } = useSelector((state: IRootState) => state.dialer);
    ...
    const queryPhoneNumber = activeContact?.direction == 'INBOUND' ? activeContact?.from : dialedNumber;

    const { data: DialedNumberLeadData } = useGetLeadByPhoneNumberQuery(queryPhoneNumber, {
        skip: !queryPhoneNumber || dialerState === 'idle',
    });
                                        ...
                                        <>
                                            <div className="w-8 h-8 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                                                <span className="text-sm">
                                                    {getInitials(
                                                        DialedNumberLeadData?.lead?.phone == activeContact?.from && activeContact?.direction == 'INBOUND'
                                                            ? DialedNumberLeadData?.lead.firstName + ' ' + (DialedNumberLeadData?.lead.lastName ?? '')
                                                            : activeContact && activeContact.direction == 'INBOUND'
                                                                ? activeContact.from
                                                                : activeContact && activeContact.from == dialedNumber
                ...
                <AnimatePresence>
                    {(!dialerMinimized || isMobile) && (
                        ...
                        >
                            ...
                            {/* Phase 2: Multi-Call Navigation Panel */}
                            {sessions.length > 1 && (
                                <motion.div className="flex overflow-x-auto p-2 bg-gray-50 border-b gap-2 hide-scrollbar">
                                    {sessions.map(s => (
                                        <button
                                            key={s.callSid}
                                            onClick={() => switchToSession(s.callSid)}
                                            className={`whitespace-nowrap flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${s.callSid === focusedCallSid ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-300'}`}
                                        >
                                            {s.status === 'incoming' && <span className="animate-pulse mr-1.5">📞 Ringing:</span>}
                                            {s.status === 'active' && !s.isMerged && <span className="text-green-300 mr-1.5">● Active:</span>}
                                            {s.isMerged && <span className="text-purple-400 mr-1.5">⛓ Conf:</span>}
                                            {s.status === 'on-hold' && <span className="text-yellow-400 mr-1.5">⏸ Held:</span>}
                                            {s.contact ? (s.contact.direction === 'INBOUND' ? formatPhoneNumber(s.contact.from) : getContactName(s.contact)) : "Call"}
                                        </button>
                                    ))}
                                </motion.div>
                            )}

                            {/* Conference Panel — shown when 2+ sessions are merged */}
                            {isConferenceActive && (
                                <motion.div
                                    initial={{ opacity: 0, y: -8 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="mx-3 mb-2 rounded-xl border border-purple-200 bg-purple-50 p-3"
                                >
                                    <div className="flex items-center gap-1.5 mb-2">
                                        <span className="text-purple-600 text-xs font-bold uppercase tracking-wider">⛓ Conference Call</span>
                                    </div>
                                    {sessions.filter(s => s.isMerged || s.status === 'active').map(s => (
                                        <div key={s.callSid} className="flex items-center justify-between py-1">
                                            <span className="text-xs text-gray-700 font-medium">
                                                {s.contact ? (s.contact.direction === 'INBOUND' ? formatPhoneNumber(s.contact.from) : getContactName(s.contact)) : "Unknown"}
                                            </span>
                                            <button
                                                onClick={() => hangUp(s.callSid)}
                                                className="text-xs bg-red-500 hover:bg-red-600 text-white px-2 py-0.5 rounded-full transition-colors"
                                            >
                                                End
                                            </button>
                                        </div>
                            ...
                            <motion.div className={`p-${isMobile ? '6' : '4'}`} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                                ...
                                {dialerState === 'incoming' && (
                                    <div className={`flex justify-center gap-3`}>
                                        ...
                                        {/* Merge — only when already on an active call */}
                                        {hasActiveCallWhileIncoming && (
                                            <motion.div whileTap={{ scale: 0.95 }}>
                                                <Button
                                                    className={`bg-purple-500 hover:bg-purple-600 text-white transition-all duration-200 ${isMobile ? 'w-20 h-20 rounded-full' : ''}`}
                                                    onClick={handleMergeCall}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? 32 : 16} height={isMobile ? 32 : 16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                        <path d="M8 7v10M16 7v10M3 12h18" />
                                                    </svg>
                                                    {!isMobile && <span className="ml-2">Merge</span>}
                                                </Button>
                                            </motion.div>
                                        )}
                                        ...
                                        <motion.div whileTap={{ scale: 0.95 }}>
                                            ...
                                            >
                                                <IconPhone size={isMobile ? 32 : 16} />
                                                {!isMobile && <span className="ml-2">{hasActiveCallWhileIncoming ? 'Switch' : 'Answer'}</span>}
                                            </Button>
                                        </motion.div>
                                    </div>
                                )}
